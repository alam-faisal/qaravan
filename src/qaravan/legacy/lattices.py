...
class SquareLattice(Lattice):
    def __init__(self, Lx, Ly, periodic_x=False, periodic_y=False):
        ...
        self.atom_positions = {
            (x + y * Lx): (x, y) for y in range(Ly) for x in range(Lx)
        }
    ...
    def plot(self, show_labels=True, figsize=(6, 6), save_path=None):
        ...
        for idx, (x, y) in self.atom_positions.items():
            ...
            if show_labels:
                ax.text(
                    x, y, str(idx), ha="center", va="center", color="white", fontsize=8
                )
...
class KagomeLattice:
    ...
    def plot(
        self, show_labels=True, show_triangles=True, figsize=(8, 6), save_path=None
    ):
        _, ax = plt.subplots(figsize=figsize)
    ...
    def spruce(self, deleted_qubits):
        ...
        self.nn_pairs = {
            pair for pair in self.nn_pairs if not deleted_set.intersection(pair)
        }
        self.triangle_terms = {
            tri for tri in self.triangle_terms if not deleted_set.intersection(tri)
        }
        ...
        self.nn_pairs = {
            tuple(sorted((index_remap[i], index_remap[j]))) for i, j in self.nn_pairs
        }
        self.triangle_terms = {
            tuple(index_remap[i] for i in tri) for tri in self.triangle_terms
        }
