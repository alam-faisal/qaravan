...
from .paulis import pauli_I, pauli_X, pauli_Y, pauli_Z
...
def is_special_orthogonal(mat):
    return bool(
        np.allclose(np.dot(mat.T, mat), np.eye(mat.shape[0]))
        and np.allclose(np.dot(mat, mat.T), np.eye(mat.shape[0]))
...
def is_special_unitary(mat):
    return bool(
        np.allclose(np.dot(mat.T.conj(), mat), np.eye(mat.shape[0]))
        and np.allclose(np.dot(mat, mat.T.conj()), np.eye(mat.shape[0]))
        and np.isclose(np.linalg.det(mat), 1.0)
    )
...
def random_su2():
    ...
    a = x[0] + 1j * x[1]
    b = x[2] + 1j * x[3]
    return np.array([[a, -b.conj()], [b, a.conj()]], dtype=complex)
...
def circuit_to_R(circ):
    ...
    Rp = np.array(
        [R.T[2 * i, :] + 1j * R.T[2 * i + 1, :] for i in range(circ.num_sites)]
    )
    return R, Rp
...
class MatchgateSimH(BaseSim):
    ...
    def initialize_state(self):
        ...
        self.instring = (
            "0" * self.num_sites if self.init_state is None else self.init_state
        )
    ...
    def probability(self, outstring):
        ...
        R = self.state
        Rp = np.array(
            [R.T[2 * i, :] + 1j * R.T[2 * i + 1, :] for i in range(self.num_sites)]
        )
        mat = exp_mat(n, labels, Rp)
...
class MatchgateSimS(BaseSim):
    ...
    def initialize_state(self):
        ...
        if self.init_state is None:
            ...
            raise ValueError(
                "init_state must be either a covariance matrix or a bitstring."
            )
    ...
    def apply_gate(self, gate):
        ...
        rot_matrix = np.array(
            [
                [
                    0.5 * np.trace(gamma1 @ gate.matrix.conj().T @ gamma2 @ gate.matrix)
