...
class PauliNoise(Noise):
    def __init__(self, strings, probs):
        assert np.abs(np.sum(probs) - 1) < 1e-3, (
            f"Probabilities must sum to 1, but got {np.sum(probs)}"
        )
    ...
    def symp_matrix(self, full=False):
        ...
        if not full:
            return np.array(
                [
                    [(-1) ** (not pauli_commute(s1, s2)) for s1 in self.strings]
            ...
            return np.array(
                [
                    [(-1) ** (not pauli_commute(s1, s2)) for s1 in strings]
                    for s2 in strings
                ]
            )
...
class PauliLindbladNoise(Noise):
    ...
    def get_kraus_for_string(self, i):
        ...
        s = self.strings[i]
        error_mat = np.emath.sqrt(self.rates[i]) * embed_operator(
            len(s),
            [i for i in range(len(s))],
            [pauli_mapping[op] for op in s],
            dense=True,
    ...
    def make_superop(self):
        superop_list = [
            self.get_superop_for_string(i) for i in range(len(self.strings))
        ]
        return (
            np.linalg.multi_dot(superop_list)
            if len(superop_list) > 1
            else superop_list[0]
        )
...
def random_pauli_channel(n, n_strings=5, c=0.1):
    ...
    probs = [
        1,
    ] + [c * np.random.rand() for _ in range(n_strings - 1)]
    probs /= np.sum(probs)
