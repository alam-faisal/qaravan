...
def decompose_site(data):
    ...
    if is_mpo:
        data = data.transpose(0, 1, 4, 2, 3, 5).reshape(
            sh[0] * sh[1] * sh[4], sh[2] * sh[3] * sh[5]
        )
    else:
...
def decompose_three_site(data):
    ...
    """
    sh = data.shape
    data = data.transpose(0, 1, 2, 4, 3, 5, 6, 7).reshape(
        sh[0], sh[1], sh[2] * sh[4], sh[3] * sh[5], sh[6], sh[7]
    )

    top_site, bottom_site = decompose_site(data)
    bottom_site = bottom_site.reshape(
        bottom_site.shape[0], sh[7], sh[2], sh[4], sh[3], sh[5]
...
def decompose_four_site(data):
    ...
    """
    sh = data.shape
    data = data.transpose(0, 2, 1, 3, 4, 6, 5, 7, 8, 9).reshape(
        sh[0] * sh[2], sh[1] * sh[3], sh[4] * sh[6], sh[5] * sh[7], sh[8], sh[9]
    )

    top_half, bottom_half = decompose_site(data)

    top_half = top_half.reshape(
        sh[8], top_half.shape[1], sh[0], sh[2], sh[1], sh[3]
    ).transpose(2, 4, 3, 5, 0, 1)
    bottom_half = bottom_half.reshape(
        bottom_half.shape[0], sh[9], sh[4], sh[6], sh[5], sh[7]
    ).transpose(2, 4, 3, 5, 0, 1)
...
class MPS(TensorNetwork):
    ...
    def compute_right_envs(self, other=None, scale=1):
        ...
        for i in range(self.num_sites - 1, 0, -1):
            right_envs[i - 1] = (
                contract_mps_env(
                    right_envs[i], self.sites[i], other.sites[i], right=True
                )
                * scale
            )
    ...
    def compute_left_envs(self, other=None, scale=1):
        ...
        for i in range(1, self.num_sites):
            left_envs[i] = (
                contract_mps_env(
                    left_envs[i - 1], self.sites[i - 1], other.sites[i - 1], right=False
                )
                * scale
            )
    ...
    def overlap(self, other, scaled=False):
        ...
        tensor = (
            ncon((self.sites[0], other.sites[0].conj()), ([1, -1, 2], [1, -2, 2]))
            * scale
        )
    ...
    def normalize(self):
        n = self.norm()
        self.sites = [
            site / np.sqrt(np.abs(n)) ** (1 / self.num_sites) for site in self.sites
        ]
    ...
    def canonize(self, center):
        ...
        if sites[0].shape[0] > 1:
            raise NotImplementedError(
                "canonization for periodic boundaries has not been implemented"
            )
    ...
    def compress(self, msvr=None, max_dim=None):
        ...
        if sites[0].shape[0] > 1:
            raise NotImplementedError(
                "compression for periodic boundaries has not been implemented"
            )
    ...
    def fast_measure(self, meas_sites):
        ...
        for meas_site in meas_sites:
            ...
            left_tensor = contract_mps_env(
                cur_left,
                self.sites[meas_site],
                op=np.array([[1, 0], [0, 0]]),
                right=False,
            )
            ...
            cur_left = contract_mps_env(
                cur_left, self.sites[meas_site], op=proj_op, right=False
            )
            cur_idx = meas_site + 1
    ...
    def fast_sample(self, num_samples: int = 1):
        ...
        for i, site in enumerate(self.sites):
            ...
            Ls = np.stack(
                [
                    np.matmul(
                        np.matmul(site[:, :, σ].conj().T, cur_lefts), site[:, :, σ]
                    )
                    for σ in range(d)
                ],
                axis=1,
            ...
            probs = np.einsum(
                "kdii->kd", Ls
            ).real  # (K, d); sums to 1 by right-canonicality
            rands = np.random.rand(K)
            sampled = (
                (rands[:, None] >= np.cumsum(probs, axis=1)).sum(axis=1).astype(np.int8)
            )
            outcomes[:, i] = sampled
    ...
    def to_vector(self):
        return contract_sites(self.sites)[0, 0, :].reshape(
            self.local_dim**self.num_sites
        )
...
class MPO(TensorNetwork):
    ...
    def __add__(self, other):
        ...
        for i, (s, o) in enumerate(zip(self.sites, other.sites)):
            if i == 0:
                site = np.zeros(
                    (1, s.shape[1] + o.shape[1], self.local_dim, self.local_dim),
                    dtype=complex,
                )
                ...
                site = np.zeros(
                    (s.shape[0] + o.shape[0], 1, self.local_dim, self.local_dim),
                    dtype=complex,
                )
                for i in range(self.local_dim):
                    for j in range(self.local_dim):
    ...
    def act(self, mps):
        ...
        for site_a, site_b in zip(self.sites, mps.sites):
            ...
            new_site = ncon((site_a, site_b), ([-1, -3, -5, 1], [-2, -4, 1])).reshape(
                ldim, rdim, mps.local_dim
            )
    ...
    def compose(self, mpo):
        ...
        for site_a, site_b in zip(self.sites, mpo.sites):
            ...
            sdim = self.local_dim
            new_site = ncon(
                (site_a, site_b), ([-1, -3, -5, 1], [-2, -4, 1, -6])
            ).reshape(ldim, rdim, sdim, sdim)
            new_sites.append(new_site)
        return type(self)(new_sites)
    ...
    def overlap(self, other, scaled=True):
        ...
        tensor = (
            ncon((self.sites[0], other.sites[0]), ([-1, -3, 1, 2], [-2, -4, 2, 1]))
            / scale
        )
        for i in range(1, self.num_sites):
            tensor = ncon((tensor, self.sites[i]), ([-1, -2, 1, -3], [1, -4, -5, -6]))
            tensor = (
                ncon((tensor, other.sites[i]), ([-1, -2, 1, -3, 2, 3], [1, -4, 3, 2]))
                / scale
            )
        return np.trace(
            tensor.reshape(
    ...
    def to_matrix(self):
        ...
        for i in range(1, self.num_sites):
            ...
            tensor = ncon(
                (tensor, self.sites[i]), ([-1, 1, -3, -5], [1, -2, -4, -6])
            ).reshape(left_dim, right_dim, local_dim, local_dim)
    ...
    def compress(self, msvr=None, max_dim=None):
        ...
        for i in range(0, self.num_sites - 1):
            ...
            sites[i] = left_tensor
            sites[i + 1] = ncon(
                (right_tensor, sites[i + 1]), ([-1, 1], [1, -2, -3, -4])
            )
...
class MPDO(MPO):
    ...
    def partial_trace(self, indices):
        ...
        for i in indices:
            ...
            if cur_idx == len(sites) - 1:
                sites[cur_idx - 1] = ncon(
                    (sites[cur_idx - 1], sites[cur_idx]), ([-1, 1, -3, -4], [1, -2])
                )
            else:
                sites[cur_idx + 1] = ncon(
                    (sites[cur_idx], sites[cur_idx + 1]), ([-1, 1], [1, -2, -3, -4])
                )
    ...
    def apply_isometry(self, site_indices, isometries):
        ...
        isometries = (
            [isometries] * len(site_indices)
            if type(isometries) == np.ndarray
            else isometries
        )
        sites = copy.deepcopy(self.sites)
        ...
        for site_idx, isometry in zip(site_indices, isometries):
            sites[site_idx] = ncon(
                (sites[site_idx], isometry, isometry.conj()),
                ([-1, -2, 1, 2], [1, -3], [2, -4]),
            )
    ...
    def project(self, site_idx, vector):
        ...
        if self.num_sites == 1:
            ...
            sites[site_idx] = ncon(
                (virt_site, sites[site_idx]), ([-1, 1], [1, -2, -3, -4])
            )
    ...
    def conj(self):
        sites = [
            copy.deepcopy(site).conj().transpose(0, 1, 3, 2) for site in self.sites
        ]
        return type(self)(sites)
...
def contract_sites(site_list):
    ...
    if len(site_list[0].shape) == 4:
        site_list = [
            site.reshape(site.shape[0], site.shape[1], site.shape[2] * site.shape[3])
...
def decimate(con_site, local_dim, msvr=None, max_dim=None, symmetric=False):
    ...
    for i in range(span - 1):
        con_site = con_site.reshape(
            con_site.shape[0], con_site.shape[1], local_dim, local_dim ** (span - i - 1)
        )
        # shape has become (b, b, d, d*d*d*...*d)
        top, con_site = decompose_site_n(
            con_site, msvr=msvr, max_dim=max_dim, symmetric=symmetric
        )
...
def sv_to_mps(sv, local_dim=2, msvr=None, max_dim=None):
    ...
    dec_sites = decimate(
        con_site, local_dim, msvr=msvr, max_dim=max_dim, symmetric=False
    )
...
def check_center(mps, desired_center, verbose=False):
    ...
    for idx, site in enumerate(mps.sites):
        ...
        assert (
            (idx < desired_center and is_left)
            or (idx > desired_center and is_right)
            or (idx == desired_center)
...
def random_mps(num_sites, max_bond_dim=4, local_dim=2, normalize=True):
    ...
    for i in range(num_sites):
        ...
        site = np.random.rand(
            left_bond_dim, right_bond_dim, local_dim
        ) + 1j * np.random.rand(left_bond_dim, right_bond_dim, local_dim)
        sites.append(site)
...
def fast_ipr(mps):
    ...
    left_up = ncon(
        [site0_up, site0_up, site0_up.conj(), site0_up.conj()], ([-1], [-2], [-3], [-4])
    )
    left_down = ncon(
        [site0_down, site0_down, site0_down.conj(), site0_down.conj()],
        ([-1], [-2], [-3], [-4]),
    )
    ...
    for i in range(1, n):
        ...
        next_list.append(
            ncon(
                [left, site_i_up, site_i_up, site_i_up.conj(), site_i_up.conj()],
...
def two_norm_fidelity(rho1, rho2):
    if type(rho1) == np.ndarray:
        ...
        raise TypeError(
            f"provided density operator is not of valid type; must be {np.ndarray} or {MPO} not {type(rho1)}"
        )
