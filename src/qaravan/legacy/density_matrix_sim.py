from qaravan.legacy.base_sim import BaseSim
from qaravan.legacy.utils import string_to_sv, pretty_print_dm
from qaravan.legacy.gates import SuperOp
...
class DensityMatrixSim(BaseSim):
    ...
    def initialize_state(self):
        ...
        if self.init_state is None:
            dm = np.zeros(
                (self.local_dim**self.num_sites, self.local_dim**self.num_sites)
            )
            dm[0, 0] = 1.0
            ...
            if len(self.init_state.shape) == 2:
                self.state = self.init_state.reshape(
                    [self.local_dim] * self.num_sites * 2
                )
            elif len(self.init_state.shape) == self.num_sites * 2:
                self.state = self.init_state
            else:
                raise ValueError(
                    "init_state must be a rank-2 tensor or a rank-2n tensor."
                )
    ...
    def apply_gate(self, gate):
        ...
        gate_indices, state_indices = (
            generate_gate_indices(gate.indices, self.num_sites),
            generate_state_indices(gate.indices, self.num_sites),
        )
    ...
    def measure(self, meas_sites):
        ...
        raise NotImplementedError(
            "Measurement not yet implemented for density matrix simulator."
        )
    ...
    def local_expectation(self, local_ops):
        ...
        for i in range(self.num_sites):
            ...
            state_indices[i] = 1
            self.state = ncon(
                (local_ops[i], self.state), ([-(i + 1), 1], state_indices)
            )

        dm = self.state.reshape(
            self.local_dim**self.num_sites, self.local_dim**self.num_sites
        )
...
def generate_gate_indices(locs, n):
    ...
    contracted = (
        sorted(numbers)[-len(numbers) // 2 :] + sorted(numbers)[: len(numbers) // 2]
    )
...
def random_dm(n):
    a = np.random.rand(2**n, 2**n) + 1.0j * np.random.rand(2**n, 2**n)
    a = a @ a.conj().T
    return a / np.trace(a)
...
def partial_trace(dm, sites, local_dim=2):
    ...
    if len(sites) == system_size:
        ...
        return ncon([dm], contraction).reshape(
            local_dim ** len(sites), local_dim ** len(sites)
        )
