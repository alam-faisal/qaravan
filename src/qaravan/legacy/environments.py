from qaravan.legacy.statevector_sim import (
    StatevectorSim,
    partial_overlap,
    all_zero_sv,
    op_action,
)
from qaravan.legacy.utils import RunContext
from qaravan.legacy.circuits import two_local_circ
...
def environment_update(
    circ, gate_idx, pre_state, post_state, target_sv, init_sv, direction="decreasing"
):
    ...
    if direction == "decreasing":
        if gate_idx == 1:
            ...
            pre_state = op_action(
                circ[gate_idx - 1].matrix.conj().T,
                circ[gate_idx - 1].indices,
                pre_state,
            )  # undo the next gate
        ...
        if gate_idx == len(circ) - 2:
            ...
            post_state = op_action(
                circ[gate_idx + 1].matrix, circ[gate_idx + 1].indices, post_state
            )  # undo the next gate
...
def environment_state_prep(
    target_sv, init_sv=None, circ=None, skeleton=None, context=None, quiet=True
):
    ...
    if context.resume:
        ...
        pre_state = op_action(
            circ[-1].matrix.conj().T, circ[-1].indices, ansatz
        )  # undo the last gate
        post_state = target_sv
    ...
    for step in range(context.step, context.max_iter):
        left_sweep = tqdm(
            reversed(range(1, len(circ))), disable=quiet, desc="Left sweep"
        )
        right_sweep = tqdm(range(len(circ) - 1), disable=quiet, desc="Right sweep")

        for i in left_sweep:
            cost, pre_state, post_state = environment_update(
                circ,
                i,
                pre_state,
                post_state,
