...
class Circuit:
    def __init__(
        self, gate_list, n=None, local_dim=2, meas_sites=None, requires_decomp=False
    ):
    ...
    def construct_layers(self):
        ...
        for gate in self.gate_list:
            ...
            while place_available and layer_ind >= 0:
                ...
                existing_indices = {
                    ind for gate in layers[layer_ind] for ind in gate.indices
                }
        ...
        for layer in self.layers:
            if not any(isinstance(obj, SuperOp) for obj in layer):
                if isinstance(nm, ThermalNoise):
                    ...
                    active_qubits = set(
                        np.concatenate([gate.indices for gate in layer])
                    )
                    idle_qubits = set(range(self.num_sites)) - active_qubits
                    noise_layer = [
                        ID(nm.local_dim, layer_time, i, nm=nm, dd=(i in idle_qubits))
                        for i in range(self.num_sites)
                    ]

                else:
                    noise_layer = [
                        ID(2, 0.0, [i for i in range(self.num_sites)], nm=nm)
                    ]
    ...
    def __getitem__(self, key):
        if isinstance(key, int):
            ...
            return Circuit(
                self.gate_list[key], n=self.num_sites, local_dim=self.local_dim
            )
        else:
    ...
    def __mul__(self, other):
        if isinstance(other, int) and other >= 0:
            ...
            raise TypeError(
                "Circuit multiplication only supports non-negative integers"
            )
    ...
    def copy(self):
        ...
        if self.layers is not None:
            new_circ.layers = [
                [g.shallow_copy() for g in layer] for layer in self.layers
            ]
...
def circ_to_mat(circ, n=None):
    ...
    for layer in circ.layers:
        missing_indices = set(range(n)) - set(
            np.concatenate([gate.indices for gate in layer])
        )
    ...
    return (
        np.linalg.multi_dot(layer_mats[::-1]) if len(layer_mats) > 1 else layer_mats[0]
    )
...
def shift_gate_list(circ, start_site):
    ...
    return [_shifted_gate(gate, start_site) for gate in circ.gate_list]
...
def heisenberg_layer(skeleton, params=None):
    params = np.random.rand(len(skeleton)) * np.pi if params is None else params
    gate_list = [
        HeisenbergCoupling(indices, params[i]) for i, indices in enumerate(skeleton)
    ]
    return Circuit(gate_list)
