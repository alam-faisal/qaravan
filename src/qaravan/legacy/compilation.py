from qaravan.legacy.utils import *
from qaravan.legacy.gates import CNOT, CNOT3, embed_operator, Gate
from qaravan.legacy.param_gates import U, U01, RZ
from qaravan.legacy.paulis import pauli_Z
from qaravan.legacy.circuits import Circuit, circ_to_mat
from qaravan.legacy.statevector_sim import StatevectorSim
...
def rz_ansatz(n, params, body=1):
    ...
    for k in range(2, body + 1):
        ...
        for i, (name, mat) in enumerate(generators):
            u = expm(-1.0j * params[len(gate_list) + i] * mat)
...
def msp_via_autodiff(
    ...
    while True:
        cost = msp_infidelity(
            init_params, cnot_skeleton, in_states, out_states, backend="torch"
        )
...
def msp_via_environments(
    in_states, ansatz, out_states, num_sweeps=100, quiet=True, threshold=2e-8
):
    term_cond = None
    cost_list = []
...
def msp_job(
    ...
    for i in range(max_instances):
        ...
        new_ansatz, cost_list, term_cond = msp_via_environments(
            in_states, ansatz, out_states, num_sweeps=max_sweeps, threshold=threshold
        )
...
def msp_optimize(vecs, target, skeleton, num_reps=10, quiet=True):
    ...
    """
    print(
        "SciPy optimizers are horrible. Use msp_via_autodiff or msp_via_environments instead."
    )
    ...
    for i in tqdm(range(num_reps)):
        ...
        result = minimize(
            msp_cost,
            x0=params,
            args=(skeleton, vecs, target),
            method="L-BFGS-B",
            options={"maxfun": 5000, "maxiter": 5000, "disp": False},
        )
