...
from .utils import pretty_print_sv
from .initializations import MPS, all_zero_mps, string_to_mps, contract_sites, decimate
    ...
    def apply_gate(self, gate, symmetric=False):
        ...
        if np.abs(inds[0] - inds[-1]) > 2 or len(inds) > 3:
            raise NotImplementedError(
                "MPS simulator currently only supports up to nn and nnn gates."
            )
...
def lift_nnn_gate(U):
    d = int(U.shape[0] ** 0.5)
    return ncon(
        (U.reshape(d, d, d, d), np.eye(d)), ([-1, -3, -4, -6], [-2, -5])
    ).reshape(d**3, d**3)
