...
def one_copy_projector_ti_subspace(local_dimension, number_sites):
    ...
    symmetry = np.concatenate(
        [np.arange(number_sites, dtype=int), np.arange(number_sites, dtype=int)]
    )
    ...
    cycle_counts = np.array(
        [
            Permutation(permutation_list[i]).cycles
            for i in range(length_permutation_list)
        ],
        dtype=int,
    )
    return_value = np.sum(float(local_dimension) ** cycle_counts) / len(
        permutation_list
    )
    return return_value
...
class TI_MPS(MPS):
    ...
    def overlap(self, other, scaled=False):
        ...
        if scaled:
            scale = np.sqrt(
                one_copy_projector_ti_subspace(self.local_dim, self.num_sites)
                ** (1 / self.num_sites)
            )
        else:
            scale = 1.0
        ...
        tensor = (
            ncon((self.sites[0], other.sites[0].conj()), ([-1, -3, 1], [-2, -4, 1]))
            * scale
        )
        for i in range(1, self.num_sites):
            tensor = ncon((tensor, self.sites[i]), ([-1, -2, 1, -3], [1, -4, -5]))
            tensor = (
                ncon((tensor, other.sites[i].conj()), ([-1, -2, 1, -3, 2], [1, -4, 2]))
                * scale
            )
        return np.trace(
            tensor.reshape(
...
def random_unitary(n, local_dim=2):
    ...
    a = np.random.rand(local_dim**n, local_dim**n) + 1j * np.random.rand(
        local_dim**n, local_dim**n
    )
    q, r = np.linalg.qr(a)
    d = np.diagonal(r)
    ph = d / np.abs(d)
...
def open_rmps_staggered(num_sites, chi, local_dim=2, distrib="isometric", param=1.0):
    ...
    for i in range(num_sites):
        ...
        if distrib == "isometric":
            site = (
                haar_random_isometry(*shape)
                .reshape(l_chi, local_dim, r_chi)
                .transpose(0, 2, 1)
            )
        elif distrib == "uniform":
            site = uniform(-1, 1, shape) + 1.0j * uniform(-1, 1, shape)
        elif distrib == "gaussian":
            site = normal(0, param, shape) + 1.0j * normal(0, param, shape)
        else:
...
def open_rmps_even(num_sites, chi, local_dim=2, distrib="isometric", param=1.0):
    ...
    for _ in range(num_sites):
        if distrib == "isometric":
            site = (
                haar_random_isometry(chi, chi, local_dim)
                .reshape(chi, local_dim, chi)
                .transpose(0, 2, 1)
            )
        elif distrib == "uniform":
            site = uniform(-1, 1, (chi, chi, local_dim)) + 1.0j * uniform(
                -1, 1, (chi, chi, local_dim)
            )
        elif distrib == "gaussian":
            site = normal(0, param, (chi, chi, local_dim)) + 1.0j * normal(
                0, param, (chi, chi, local_dim)
            )
        else:
            raise ValueError(f"{distrib} is not a valid distribution for site tensors")
        sites.append(site)
...
def rand_mpo(n, chi, boundary="open", local_dim=2):
    if boundary == "open":
        sites = (
            [np.random.rand(1, chi, local_dim, local_dim)]
            + [np.random.rand(chi, chi, local_dim, local_dim) for _ in range(n - 2)]
            + [np.random.rand(chi, 1, local_dim, local_dim)]
        )
