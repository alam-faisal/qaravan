...
_QUBIT_GATES = [I(0), X(0), Y(0), Z(0), H(0), S(0), Sdg(0), T(0), Tdg(0), SX(0),
                CNOT([0, 1]), CZ([0, 1]), SWAP([0, 1]), iSWAP([0, 1])]
...
_QUTRIT_GATES = [X01(0), SX01(0), X12(0), SX12(0), H01(0), SDG01(0),
                  SWAP3([0, 1]), CNOT3([0, 1])]
...
class _MinState(State):
    ...
    def sample_and_collapse(self, s): return ("0" * len(s), self)
    def overlap(self, o): return 1.0

class _MinSim(Simulator):
    def _validate_state(self, state):
        if not isinstance(state, _MinState):
            raise IncompatibleStateError
    def translate_gate(self, gate): return gate.matrix
    def _apply_translated_gate(self, state, tg): pass
