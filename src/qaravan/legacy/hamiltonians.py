...
def terms_to_matrix(terms, num_sites, dense=True, spin_half=False):
    ...
    for coeff, active_sites, local_ops in terms:
        H += coeff * embed_operator(
            num_sites,
            active_sites,
            [pauli_mapping[op] for op in local_ops],
            factor=spin_half,
        )
    if dense:
        return H.toarray()
...
class Hamiltonian:
    def __init__(
        ...
        if locality == "nn":
            ...
            for ops, nn_strength, nnn_strength in zip(
                coupling_types, nn_strengths, nnn_strengths
            ):
            ...
            for ops, strength in zip(coupling_types, coupling_strengths):
                for pairs in self.lattice.get_connections("a2a"):
                    self.terms.append(
                        (
                            strength / np.abs(pairs[0] - pairs[1]),
                            pairs,
                            [ops[0], ops[1]],
                        )
                    )
        ...
        for op, strength in zip(field_types, field_strengths):
            if np.isscalar(strength):  # uniform field
                ...
                if len(strength_array) != self.num_sites:
                    raise ValueError(
                        f"Field strength array length {len(strength_array)} must match "
    ...
    def matrix(self, dense=False):
        return terms_to_matrix(
            self.terms, self.num_sites, dense=dense, spin_half=self.spin_half
        )
    ...
    def trotter_circ(self, step_size, num_steps, order=1):
        ...
        if order == 1:
            layer_list = [
                self.layer_from_group(group, step_size, self.num_sites)
                for group in self.op_grouping
            ]
            ...
            l0h = self.layer_from_group(
                self.op_grouping[0], step_size / 2, self.num_sites
            )
            ...
            raise NotImplementedError(
                "Only first and second order Trotter decomposition is supported"
            )
    ...
    def layer_from_group(self, group, time, num_sites):
        ...
        if group in ["even", "odd"]:
            generator_mat = sum(
                [
                    ...
                    * embed_operator(
                        2,
                        [0, 1],
                        [pauli_mapping[t[0]], pauli_mapping[t[1]]],
                        factor=self.spin_half,
                    )
            ...
            c = (
                self.coupling_strengths[self.coupling_types.index(group)]
                if len(group) == 2
                else self.field_strengths[self.field_types.index(group)]
            )
            generator_mat = c * embed_operator(
                len(group),
                [i for i in range(len(group))],
                [pauli_mapping[op] for op in group],
                factor=self.spin_half,
            )
        ...
        if len(group) == 1:
            ...
            raise NotImplementedError(
                "Only field and nearest-neighbor couplings are supported"
            )
...
class XYNNN(Hamiltonian):
    ...
    def layer_from_group(self, group, time, num_sites):
        if group in ["even", "odd"]:
            generator_mat = sum(
                c
                * embed_operator(2, [0, 1], [pauli_mapping[t[0]], pauli_mapping[t[1]]])
                for c, t in zip(self.coupling_strengths, self.coupling_types)
            )
        else:
            generator_mat = sum(
                ...
                * embed_operator(
                    3,
                    [0, 1, 2],
                    [pauli_mapping[t[0]], pauli_mapping[t[1]], pauli_mapping[t[2]]],
                )
                for c, t in zip(self.coupling_strengths, self.coupling_types)
            )
...
class ChiralHeisenberg(Hamiltonian):
    def __init__(
        self, row_layout, jx=1, jy=1, jz=1, chiral_strength=0.01, h=0, sp=None
    ):
    ...
    def pairwise_terms(self, pairs):
        return [
            (j, p, op)
            for p in pairs
            for (j, op) in zip(self.coupling_strengths, self.coupling_types)
        ]
    ...
    def chiral_terms(self, triplet):
        ...
        return [
            (
                self.chiral_strength
                if op in ["xyz", "yzx", "zxy"]
                else -self.chiral_strength,
                triplet,
                op,
            )
            for op in ops
    ...
    def primitive_matrices(self):
        ...
        triangle_terms = self.pairwise_terms(
            [(0, 1), (1, 2), (0, 2)]
        ) + self.chiral_terms((0, 1, 2))
        tmat = sum(
            [
                coeff
                * embed_operator(
                ...
                for coeff, sites, local_ops in triangle_terms
            ]
        )

        edge_terms = self.pairwise_terms(
            [
                (0, 1),
            ]
        )
        emat = sum(
            [
                ...
                * embed_operator(
                ...
                for coeff, sites, local_ops in edge_terms
            ]
        )
    ...
    def grouped_terms(self):
        ...
        triangle_edges = [
            set(pair)
            for t in self.lattice.triangle_terms
            for pair in [(t[0], t[1]), (t[1], t[2]), (t[0], t[2])]
        ]
        edge_terms = [
            pair for pair in self.lattice.nn_pairs if set(pair) not in triangle_edges
        ]
        return [(indices, tmat) for indices in self.lattice.triangle_terms] + [
            (indices, emat) for indices in edge_terms
        ]
...
class ToricCode(Hamiltonian):
    ...
    def __str__(self):
        return (
            f"Toric Code Hamiltonian on {self.lattice.Lx}×{self.lattice.Ly} "
