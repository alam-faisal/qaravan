...
def test_contract_sites_mpdo():
    ...
    sites = (
        ...
        + [np.random.rand(3, 3, 2, 2)] * 1
        + [np.random.rand(3, 5, 2, 2)]
    )
    c_site = contract_sites(sites)
    assert c_site.shape == (4, 5, 2**6), (
        f"Expected shape (4, 5, 64), got {c_site.shape}"
    )
...
def test_decimate_mps():
    ...
    assert np.allclose(c_site, c_site2), (
        "Decimated sites don't reconstruct original contracted site"
    )
    assert all(s.shape == d.shape for s, d in zip(sites, dec_sites)), (
        "Decimated site shapes don't match original"
    )
...
def test_decimate_mpdo():
    ...
    assert np.allclose(c_site, c_site2), (
        "Decimated MPDO sites don't reconstruct original"
    )
    assert all(s.shape == d.shape for s, d in zip(sites, dec_sites)), (
        "Decimated MPDO shapes don't match"
    )
...
def test_environments():
    ...
    for i in range(n - 1):
        assert np.allclose(ncon((le[i + 1], re[i]), ([1, 2], [1, 2])), norm), (
            f"Environment overlap at {i} doesn't match norm"
        )
...
def test_environments_two_mps():
    ...
    for i in range(n - 1):
        ...
        assert np.allclose(env_overlap, true_overlap), (
            f"Environment overlap at {i} doesn't match true overlap"
        )
...
def test_overlap():
    ...
    assert np.allclose(mps_overlap, direct_overlap), (
        f"MPS overlap {mps_overlap} doesn't match direct overlap {direct_overlap}"
    )
...
def test_fast_measure():
    ...
    for num_samples in num_sample_opts:
        ...
        assert np.linalg.norm(
            exact_density_vec - mps_density_vec, ord=2
        ) < l2_threshold(num_samples, 2 ** len(meas_sites), delta=1e-12), (
...
def test_one_local_expectation():
    ...
    assert np.allclose(true_exp, mps_exp), (
        f"MPS expectation {mps_exp} doesn't match exact value {true_exp}"
    )
...
def test_mps_measurement_probabilities():
    ...
    left_tensor = contract_mps_env(
        cur_left, mps.sites[0], mps.sites[0], op=np.array([[1, 0], [0, 0]]), right=False
    )
    prob_0_mps = ncon((left_tensor, mps.right_envs[0]), ([1, 2], [1, 2])).real

    assert np.allclose(prob_0_mps, exact_prob_0), (
        f"MPS prob(0) {prob_0_mps} doesn't match exact {exact_prob_0}"
    )
    print(
        "Test passed: MPS measurement probabilities match exact statevector probabilities."
    )
