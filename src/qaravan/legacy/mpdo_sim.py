from qaravan.legacy.gates import SuperOp, ID
from qaravan.legacy.noise import gate_time
...
def layer_to_superop(layer, span, start_idx, nm=None, name="S", time=0.0):
    ...
    id_gates = [
        ID(local_dim=local_dim, time=0.0, indices=[idx]) for idx in missing_indices
    ]
    ...
    quarters = [
        sum(
            [
                sublist[i * len(sublist) // 4 : (i + 1) * len(sublist) // 4]
    ...
    return SuperOp(
        name, span, result.transpose(*new_order), start_idx=start_idx, time=time
    )
...
def noise_superop(layer, span, start_idx, nm, name="N"):
    ...
    noise_layer = [
        ID(nm.local_dim, max(nm.coupling * layer_time, gate_times[i]), i)
        for i in range(span)
    ]
    return layer_to_superop(
        noise_layer, span, start_idx, nm=nm, name=name, time=np.array(gate_times)
    )
...
def prod(superop1, superop2, name="S"):
    ...
    matrix = np.tensordot(
        superop1.matrix,
        superop2.matrix,
        axes=(range(num_indices // 2, num_indices), range(num_indices // 2)),
    )
    return SuperOp(
        name,
        superop1.span,
        matrix,
        start_idx=superop1.start_idx,
        time=superop1.time + superop2.time,
    )
...
def three_local(superop, site):
    ...
    output = ncon(
        (superop, site),
        ([-2, -4, -6, -1, -3, -5, 2, 4, 6, 1, 3, 5], [1, 2, 3, 4, 5, 6, -7, -8]),
    )
    return output
...
def superop_action(mpdo, gate, nm=None):
    ...
    inds = gate.indices
    superop = (
        gate.matrix if type(gate) == SuperOp else gate.to_superop()
    )  # gate.to_superop(nm)
    ...
    if len(inds) == 1:
        ...
        data = ncon(
            (sites[inds[0]], sites[inds[1]]), ([-1, 1, -2, -3], [1, -4, -5, -6])
        )
        ...
        data = ncon(
            (sites[inds[0]], sites[inds[1]], sites[inds[2]]),
            ([-7, 1, -1, -2], [1, 2, -3, -4], [2, -8, -5, -6]),
        )
        ...
        data = ncon(
            (sites[inds[0]], sites[inds[1]], sites[inds[2]], sites[inds[3]]),
            ([-9, 1, -1, -2], [1, 2, -3, -4], [2, 3, -5, -6], [3, -10, -7, -8]),
        )
        data = four_local(superop, data)
        sites[inds[0]], sites[inds[1]], sites[inds[2]], sites[inds[3]] = (
            decompose_four_site(data)
        )
...
def nt_sim(circ, **kwargs):
    ...
    """
    init_mpdo, nm, msvr, max_dim, dm, debug, quiet = (
        kwargs.get("init_mpdo"),
        kwargs.get("nm"),
        kwargs.get("msvr"),
        kwargs.get("max_dim"),
        kwargs.get("dm", False),
    ...
    mpdo = (
        all_zero_mpdo(circ.num_sites, circ.local_dim)
        if init_mpdo is None
        else copy.deepcopy(init_mpdo)
    )
    circ = circ.build(nm)
    # print(circ)
