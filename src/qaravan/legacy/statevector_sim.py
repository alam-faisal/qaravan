...
from qaravan.legacy.utils import string_to_sv, pretty_print_sv
from qaravan.legacy.gates import proj_up, proj_down
from qaravan.legacy.circuits import Circuit
from qaravan.legacy.paulis import pauli_mapping
import numpy as np
import torch
...
class StatevectorSim(BaseSim):
    ...
    def initialize_state(self):
        ...
        if self.init_state is None:
            ...
            raise ValueError(
                "init_state must be a NumPy array, Torch tensor, or a bitstring"
            )
    ...
    def local_expectation(self, local_ops):
        ...
        return ncon(
            (self.state.conj(), right_state),
            (
                [i for i in range(1, self.num_sites + 1)],
                [i for i in range(1, self.num_sites + 1)],
            ),
        ).real
    ...
    def pauli_expectation(self, pauli_string):
        ...
        self.run(progress_bar=False)
        assert self.backend == "numpy", (
            "Pauli expectation currently only supported for numpy backend"
        )
...
def random_sv(num_sites, local_dim=2):
    sv = np.random.rand(local_dim**num_sites) + 1j * np.random.rand(
        local_dim**num_sites
    )
...
def measure_sv(sv, meas_sites, shots=1):
    ...
    basis_outcomes = [
        np.binary_repr(i, width=len(meas_sites)) for i in range(len(probs))
    ]
    if shots == 1:
...
def measure_and_collapse_sv(sv, meas_sites, local_dim=2):
    ...
    collapsed = (
        collapsed.reshape(local_dim ** (n - len(meas_sites)))
        if sv.ndim == 1
        else collapsed
    )
...
def pauli_expectation_sv(sv, pauli_string):
    ...
    assert len(pauli_string) == n, (
        "Pauli string length must match number of qubits in statevector"
    )
    ...
    return ncon(
        (tensor.conj(), right_tensor),
        ([i for i in range(1, n + 1)], [i for i in range(1, n + 1)]),
    ).real
